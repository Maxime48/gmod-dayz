...
function WSWITCH:DrawBarBg(x, y, w, h, col, selected)
   ...
   if LocalPlayer():Frags() >= PHDayZ.Player_BountyKillsReq then
		role = 4
   elseif LocalPlayer():Frags() >= PHDayZ.Player_BanditKillsReq then
		role = 3
...
function WSWITCH:DrawWeapon(x, y, c, wep)
   ...
   if wep:GetClass() == "weapon_physgun" then
		cl1, am1 = -1, -1
		wep.Slot = 0
		wep.PrintName = "Physics Gun"
   else
		cl1, am1 =  wep:Clip1(), (wep.Ammo1 and wep:Ammo1() or 0)
   end
