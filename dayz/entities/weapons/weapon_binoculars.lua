...
if ( CLIENT ) then
	...
	function SWEP:DrawHUD()
		...
		local zoom=self:GetNWInt("zoom")
				...
				draw.DrawText(dist.." ft", "ChatFont", centerX,centerY+135-zoom, Color(255, 220, 0, 255),1)
				if(LocalPlayer():GetEyeTraceNoCursor().Entity && IsValid((LocalPlayer():GetEyeTraceNoCursor().Entity)) && !(LocalPlayer():GetEyeTraceNoCursor().Entity:GetClass() == "player") )then
					...
					draw.DrawText(str, "ChatFont", centerX,centerY+120-zoom, Color(255, 220, 0, 255),1)
...
if (CLIENT) then return end 
	...
	self.Owner:EmitSound( "binoculars/binoculars_zoommax.wav", 75, 100, 0.1 )
...
function SWEP:Zoom(cont)
	...
	self.Owner:SetFOV( self.ZoomDistCurrent, 0 )
	self.Owner:EmitSound( "binoculars/binoculars_zoomin.wav", 75, 100, 0.1 )
...
self.Owner:EmitSound( "binoculars/binoculars_zoomout.wav", 75, 100, 0.1 )
