...
ENT.EngineDefaultPower = 10000 --20000
ENT.EnginePower = 7500 --15000
...
function ENT:Use( activator, caller )
    ...
    if (activator.LstEntr < CurTime() ) and self.Destroyed == false && activator:IsPlayer() then
        ...
        if speed > 50 then
            activator:Tip(3, "You cannot enter a moving vehicle!", Color(255,255,0))
            return false
        end

        self:LoadSounds()

        activator:EmitSound("doors/handle_pushbar_open1.wav", 75, 100, 0.5)

        activator:DoModelProcess(self:GetModel(), "Entering Helicopter", 2, "", 0, "doors/default_stop.wav", true, function(ply)
            if !IsValid(activator) or !activator:Alive() then return end
            if !IsValid(self) then return end

            speed = self:GetVelocity():Length()
            if self:GetParent():IsValid() then
                speed = self:GetParent():GetVelocity():Length()
            end
            ...
            if speed < 50 then 
                ...
                for k, v in pairs(self.Seats) do
                    if IsValid(v) then
                        --if !activator:IsVIP() and k == 1 then 
                           -- continue 
                        --end
                        
                        seatNum = self:GetNextEmptySeat( k, activator )
                        if !self:HasDriver() then seatNum = 1 end
                    end
                end
...
function ENT:OnTakeDamage(dmg)
	...
	if dmg:GetAttacker():IsPlayer() and dmg:GetAttacker():GetSafeZoneEdge() then return end
...
function ENT:Think()
    ...
    if mCompass_Settings then
        ...
        if self.nextAlert < CurTime() then
            ...
            self.MarkerID = Adv_Compass_AddMarker(true, self, CurTime() + 25, Color(255,0,190,255), nil, nil, "Helicopter" )
        end
    end
...
function ENT:GetNextEmptySeat( start, ply )
	...
	for i = start, (4 + start) do
		...
		--if !ply:IsVIP() and slot == 1 then slot = slot + 1 end
    ...
    for k,v in pairs(self.Seats) do
        if IsValid(v) then
            ...
            if driver != NULL && IsValid(driver) && driver:InVehicle() then
				...
				if driver:KeyDown( IN_ATTACK2 ) and ( driver.SwapDel or 0 ) < CurTime() then
